...
def run_catchup(supabase: Client) -> int:
    """
    Fetch all existing rows from the 'votes' table (ordered by insertion
    time) and pass each through vote_processor.process_vote().

    Votes already in the local dedup set are silently skipped, so this
    function is safe to call at any time without risk of double-counting.

    Args:
        supabase: An authenticated Supabase client.

    Returns:
        Number of previously-unprocessed votes caught up.
    """

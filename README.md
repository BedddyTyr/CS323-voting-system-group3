...
### Emmanuel Y. Sonquipal II
This laboratory activity provided practical insight into how fault tolerance is achieved through system architecture rather than relying solely on runtime error-handling mechanisms. By separating the ingestion layer from the processing layer, the system continued to accept and store votes even when worker services became unavailable. This behavior remained consistent throughout the fault-injection tests, reinforcing the architectural principle that data acceptance and data processing should be handled by independent components.The Realtime configuration issue encountered during the activity—specifically the need to manually execute an ALTER PUBLICATION command and configure REPLICA IDENTITY FULL, settings unavailable through the dashboard interface—highlighted the limitations of platform abstractions. It emphasized the importance of understanding the underlying PostgreSQL replication mechanisms when troubleshooting issues that arise beyond the platform’s exposed configuration options.Furthermore, comparing vote records from the edge logs, database tables, and worker outputs verified that all components eventually reached a consistent state. Despite temporary inconsistencies introduced during failure simulations, the system successfully converged on the correct final result, demonstrating the effectiveness of its eventual consistency model.
